...
- Be able to compile DataPreparationEDA_WDS.rmd
...
```{r readAndChangeNames}
radioData <- read.csv("data/Survey_results_final.csv")
radioData <- radioData %>%
...
radioData <- radioData %>% filter(Age != "", Gender != "", Education != "", Income != "", Sirius != "", Wharton != "", Worktime != "")

radioData <- radioData %>% filter(
  Age != "female",
  Age != "223",
  Age != "4",
  )
index <- which(radioData$Age == "27`") ##find
radioData$Age[index] <- "27"##changeTo
radioData$Age[index]

index2 <- which(radioData$Age == "Eighteen (18)")
radioData$Age[index2] <- "18"

sort(unique(radioData$Age))
sort(unique(radioData$Gender))
sort(unique(radioData$Education))
sort(unique(radioData$Income))
sort(unique(radioData$Sirius))
sort(unique(radioData$Wharton))
sort(unique(radioData$Worktime))
```
...
sirius_data <- fread("data/Survey_results_final.csv", select = c("Answer.Age", "Answer.Education", "Answer.Gender", "Answer.HouseHoldIncome", "Answer.Sirius Radio", "Answer.Wharton Radio", "WorkTimeInSeconds"), stringsAsFactors=FALSE, encoding = 'UTF-8', fill = TRUE, header = TRUE)
# Rename Columns
